...
class Simhash(object):
    ...
    def __init__(self, value, f=64, reg=r'[\w\u4e00-\u9fcc]+', hashfunc=None):
        ...
        #self.wordsWeightMap = dataUtil.loadWordsWeightsMap()
        self.wordsWeightMap = None
    ...
    def weight(self, item):
        if self.wordsWeightMap is not None:
            if item in self.wordsWeightMap:
                return self.wordsWeightMap[item]
            else:
                return self.wordsWeightMap['UNK']
        else:
            return 1
    ...
    def similarity(self, another):
        ans = self.distance(another)
        return (self.f - ans) / self.f
