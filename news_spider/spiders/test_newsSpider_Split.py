...
class ArticleSpider(CrawlSpider):
    ...
    def parse(self,response):
        ...
        try:
            ...
            if response.url == self.last_url:
                print "这次%s与上次的url%s相同"%(response.url,self.last_url)
                self.update_config()
                return
            ...
            for one_link in one_page_links:
                ...
                # 获得下一页
                #print "文章数:",self.papers
            next_page_url = response.xpath(self.website_config['next_path_xpath'])
    ...
    def parse_content(self,response):
        ...
        try:
            print "content_url = {url}".format(url = response.url)
